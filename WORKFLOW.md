...
src/              unpacked EPUB — THE SOURCE OF TRUTH
  ...
  toc.ncx           navigation (playOrder kept by tools/ncx_playorder.py)
  cover.xhtml       the cover image; OPS/titlepage.htm the title page (Tibetan title,
                    house, edition line — nothing else)
  key.xhtml         How to read this book — the legend of every mark (back matter)
  acknowledgements.htm  the colophon: provenance, sources, version, URL and QR (last)
  dkarchag.htm      GENERATED by tools/dkarchag.py from toc.ncx — the Tibetan contents,
                    at the front, scaling with the reader's text; English glosses
                    on request, from resources/glosses.tsv
  jewels.htm        GENERATED by tools/jewels.py — every landing jewel
  mchan.htm         Annotations — the editor's notes in full; each ࿙ mark in
                    the text links here, each entry jumps back
  stylesheet.css    the one stylesheet; toggle.js the one script
  OPS/              10 prayer documents, one per section of the printed book
                    (p004-084 … p624-744, zur-yig), plus titlepage.htm
...
tools/            pack.sh, check.py, nav.py, ncx_playorder.py, jewels.py, …
